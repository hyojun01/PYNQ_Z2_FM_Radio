...
typedef ap_fixed<16, 1> out_data_t;
typedef hls::axis<out_data_t, 0, 0, 0> out_axis_t;
typedef ap_fixed<32, 10> in_data_t;
typedef ap_fixed<32, 10> acc_t;
typedef hls::axis<in_data_t, 0, 0, 0> in_axis_t;
typedef ap_uint<2> count_t;

void low_pass_filter_second(hls::stream<in_axis_t>& input, hls::stream<out_axis_t>& output) {
...
#pragma HLS INTERFACE mode = ap_ctrl_none port = return 
    ...
    static in_data_t shift_register[DECIMATION_FACTOR][22];
    ...
    if (count == DECIMATION_FACTOR - 1) {
        output_temp.data = (out_data_t)sum;
        output_temp.keep = -1;
        output_temp.strb = -1;
